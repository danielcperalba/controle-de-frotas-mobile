...
interface DetectivaCardProps {
  manutencoes: Manutencao[];
}

export default function DetectivaCard({ manutencoes }: DetectivaCardProps) {
  const manutencoesFiltradas = manutencoes
    .filter(manutencao => manutencao.tp_manutencao === 'Detectiva')
    ...
    <View style={styles.card}>
      ...
      <View style={styles.content}>
        {manutencoesFiltradas.length > 0 ? (
          ...
          <Text style={styles.value}>Nenhuma manutenção detectiva encontrada.</Text>
        )}
      </View>
    </View>
  );
